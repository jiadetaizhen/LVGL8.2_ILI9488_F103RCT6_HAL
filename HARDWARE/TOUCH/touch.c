...
//	GPIO_Init(GPIOC, &GPIO_InitStructure);
    ...
    __HAL_RCC_AFIO_CLK_ENABLE(); // 根据需要启用AFIO时钟

    // 配置PC3, PC0和PC13为推挽输出
    GPIO_InitStruct.Pin = GPIO_PIN_3 | GPIO_PIN_0 | GPIO_PIN_13; // PC3, PC0, PC13
    ...
    // 配置PC10和PC2为上拉输入
    GPIO_InitStruct.Pin = GPIO_PIN_10 | GPIO_PIN_2; // PC10, PC2
