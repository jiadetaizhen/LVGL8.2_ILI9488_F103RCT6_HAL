...
void LCD_Fill(uint16_t sx,uint16_t sy,uint16_t ex,u16 ey,uint16_t* color)
{  	
	u16 height, width;
	width = ex - sx + 1;            
    height = ey - sy + 1;           
...
{
	LCD_Fill(x1,y1,x2,y2,(uint16_t*)POINT_COLOR);
}
...
void Fill_Triangel(u16 x0,u16 y0,u16 x1,u16 y1,u16 x2,u16 y2)
{
  ...
  if (y0 > y1) 
		...
		else if(x2 > b)
    {
		...
		LCD_Fill(a,y0,b,y0,(uint16_t*)POINT_COLOR);
    return;
	}
	dx01 = x1 - x0;
	dy01 = y1 - y0;
    ...
    if(a > b)
    {
		...
		LCD_Fill(a,y,b,y,(uint16_t*)POINT_COLOR);
	}
	sa = dx12 * (y - y1);
	sb = dx02 * (y - y0);
	for(; y<=y2; y++) 
